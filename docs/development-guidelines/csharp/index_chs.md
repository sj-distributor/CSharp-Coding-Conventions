...
### **要** 优先使用项目中预先设定的代码样式和风格，即便它们与本指南不同 conform to pre-existing code styles in a project, even when they differ from this guide

### **避免** 在单个代码库中混合多种编码样式和风格

### **要** 利用工具配置文件在开发人员的环境中自动控制样式和结构
...
### **要** 使用语句（或指令）删除未使用的或冗余的

### **不要** 将多行语句的大括号放在与自己同一行上
...
### **要** 使用显式修饰符，即使它与默认访问级别匹配

示例：默认情况下，字段声明是“private”，但添加显式private修饰符清楚地表明了这一意图。

### **要** 对只能在类内部访问的类型或成员使用私有修饰符，对所有其他类型或成员都使用公共修饰符。
...
### **要** 在命名方法、变量等时，使用能记住的命名和可识别的模式来表达意图
